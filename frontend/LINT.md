...
#### 功能组件组织清单
* `components`
    * `elements`: 业务无关的一些简易元素的显示组件。
    * `forms`: 业务无关的表单编辑元素的显示组件。
    * `features`: 业务无关的一些功能性或重型组件。
* `layouts`
    * `data`: 重型的业务相关组件，提供某一个具体业务场景的高内聚封装。
    * `displays`: 业务相关的表单显示类组件。
    * `editors`: 业务相关的表单编辑器组件。
    * `layouts`: 重型布局组件和框架组件。
    * `pages`: 复用的页面组件。
    * `topbars`: 业务相关的、顶栏上使用的各类组件。
    * `web-modules`: 在module中由前端绘制实现的功能组件，同时提供组件和VCA注入。
