...
import com.heerkirov.hedge.server.dao.system.HistoryRecords
...
class HistoryRecordManager(private val data: DataRepository) {
    ...
    private val history: EnumMap<HistoryRecord.Type, MutableList<HistoryRecord>> = EnumMap(HistoryRecord.Type::class.java)
    private val frequent: EnumMap<HistoryRecord.Type, CountSet<String>> = EnumMap(HistoryRecord.Type::class.java)
    private val sequenceIds: EnumMap<HistoryRecord.Type, Long> = EnumMap(HistoryRecord.Type::class.java)
    private var loaded: Boolean = false

    fun getRecent(type: HistoryRecord.Type, limit: Int? = null): List<String> {
    ...
    fun getFrequent(type: HistoryRecord.Type, limit: Int? = null): List<Pair<String, Int>> {
        checkAndLoadRecordFromDB()
    ...
    @Synchronized
    fun push(type: HistoryRecord.Type, key: String) {
        checkAndLoadRecordFromDB()
    ...
    @Synchronized
    fun clear(type: HistoryRecord.Type? = null) {
        if(type == null) {
            history.clear()
            frequent.clear()
            sequenceIds.clear()
